 ...
 * Mar 29, 2006
 * Snow Tsui version : TODO: Change the cross product to VFXEpoch based calculation
 */
void draw_3d_arrow(const float base[3], const float point[3], float arrow_head_length)
{
    ...
    VFXEpoch::Vector3Df w(point[0]-base[0], point[1]-base[1], point[2]-base[2]);
    double len = w.length();
    ...
    VFXEpoch::Vector3Df u = Vector3Df::cross(w, VFXEpoch::Vector3Df(0,1,0));

    // If vector is parallel to the y-axis, use the x-axis
    if (u.length() == 0)
        u = Vector3Df::cross(w, VFXEpoch::Vector3Df(1,0,0));

    // get a vector perp to w and u
    VFXEpoch::Vector3Df v = Vector3Df::cross(w, u/u.length());
    v = v/v.length();
    ...
    VFXEpoch::Vector3Df arrow1, arrow2;
    arrow1.m_x = point[0] + arrow_head_length * (v.m_x - w.m_x);
    arrow1.m_y = point[1] + arrow_head_length * (v.m_y - w.m_y);
    arrow1.m_z = point[2] + arrow_head_length * (v.m_z - w.m_z);
    arrow2.m_x = point[0] + arrow_head_length * (-v.m_x - w.m_x);
    arrow2.m_y = point[1] + arrow_head_length * (-v.m_y - w.m_y);
    arrow2.m_z = point[2] + arrow_head_length * (-v.m_z - w.m_z);

    glBegin(GL_LINES);
    glVertex3f(base[0],   base[1],   base[2]);
    glVertex3f(point[0],  point[1],  point[2]);
    glVertex3f(point[0],  point[1],  point[2]);
    glVertex3f(arrow1.m_x, arrow1.m_y, arrow1.m_z);
    glVertex3f(point[0],  point[1],  point[2]);
    glVertex3f(arrow2.m_x, arrow2.m_y, arrow2.m_z);
    glEnd();
...
void draw_2d_arrow(const VFXEpoch::Vector2Df base, const VFXEpoch::Vector2Df point, float arrow_head_length)
{
    //glPushAttrib(GL_CURRENT_BIT|GL_ENABLE_BIT|GL_LINE_BIT);
    //glDisable(GL_LIGHTING);

    VFXEpoch::Vector2Df w = point-base;
    double len = w.length();

    if (len==0) {
        glBegin(GL_POINTS);
        glVertex2f(base.m_x, base.m_y);
        glEnd();
        return;
    }
    ...
    //	                     -1  0
    VFXEpoch::Vector2Df u = VFXEpoch::Vector2Df(1*w.m_y, -1*w.m_x);
    u = u/u.length();

    // v = w - 90 (in fact v=-u)
    VFXEpoch::Vector2Df v = VFXEpoch::Vector2Df(-1*w.m_y, 1*w.m_x);
    v = v/v.length();
    ...
    // arrow head points
    VFXEpoch::Vector2Df arrow1, arrow2;
    ...
    glVertex2f(point.m_x, point.m_y);
    glVertex2f(arrow1.m_x, arrow1.m_y);
    glVertex2f(point.m_x, point.m_y);
    glVertex2f(arrow2.m_x, arrow2.m_y);
    glEnd();
...
void init(const char *windowtitle, int *argc, char **argv, int width = 720, int height = 480)
{
    winwidth = width;
    winheight = height;
    glutInit(argc, argv);
