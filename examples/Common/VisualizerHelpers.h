...
namespace VFXEpoch {
  ...
  namespace Gluvi {
    ...
    struct TargetOrtho3D: public Camera {
      ...
      action_mode; // @@@@ WHAT ABOUT ZOOMING??? IS WIDTH ALWAYS A FUNCTION OF DIST?
      int oldmousex, oldmousey;
    ...
    void draw_3d_arrow(const float base[3],
      const float point[3], float arrow_head_length = 0);
    void draw_2d_arrow(const VFXEpoch::Vector2Df base,
      const VFXEpoch::Vector2Df point, float arrow_head_length);
    void draw_coordinate_grid(float size = 1, int spacing = 10);
    void draw_text(const float point[3],
      const char * text, int fontsize = 12);

    // call init first thing
    void init(const char * windowtitle, int * argc, char * * argv, int width, int height);
